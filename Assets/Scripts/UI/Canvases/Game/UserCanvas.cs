...
/*    private void InitializeDataToDictionary()
    {
        _translateKeywords = new Dictionary<string, string>
        {
            { "Profession", "Профессия" },
            { "Skills", "Способность" },
            { "Sex", "Пол" },
            { "Age", "Возраст" },
            { "SexualOrientation", "Сексуальная орентация" },
            { "Health", "Здоровье" },
            { "Hobby" , "Хобби"},
            { "CharacterTraits" , "Черты характера"},
            { "Phobia" , "Фобия"},
            { "Additionally" , "Дополнительно"},
            { "SmallLuggage" , "Малый багаж"},
            { "BigLuggage" , "Большой багаж"},
        };
    }*/
    ...
    private void SetUserData()
    {
        ...
        Instantiate($"{_translateKeywords[Keys.Sex]} / {_translateKeywords[Keys.Age]} / {_translateKeywords[Keys.SexualOrientation]}",
            $"{_data.GetData[Keys.Sex].Text[GetRandomParams(_data.GetData[Keys.Sex].Text.Count)]} / " +
